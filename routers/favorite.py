...
async def check_favorite(
    ...
    return success_response(
        message="检查收藏状态成功", data=FavoriteCheckResponse(isFavorite=is_favorited)
    )
...
async def remove_favorite(
    ...
    if not result:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="收藏记录不存在"
        )
...
async def get_favorite_list(
    ...
    favorite_list = [
        FavoriteNewsItemResponse(
            **news.__dict__, favoriteTime=favorite_time, favoriteId=favorite_id
        )
