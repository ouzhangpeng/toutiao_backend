...
async def get_categories(
    skip: int = 0, limit: int = 10, db: AsyncSession = Depends(get_db)  # type: ignore
):
...
async def get_news_list(
    category_id: int = Query(..., alias="categoryId"),  # type: ignore
    page: int = Query(1, alias="page"),  # type: ignore
    page_size: int = Query(10, alias="pageSize", le=100),  # type: ignore
...
async def get_news_detail(
    news_id: int = Query(..., alias="id"),  # type: ignore
    db: AsyncSession = Depends(get_db),  # type: ignore
):
    # 获取新闻详情 + 浏览量+1 + 相关新闻
    ...
    related_news = await news_cache.get_related_news(
